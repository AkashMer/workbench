...
and worst candidates. Session `755434585` / Mouse `730760270` and
Session `756029989` / Mouse `734865738` were picked for applying the
estimation pipeline since both had sufficient number of neurons in all
areas with total number of neurons sitting within the range of 100s to
tens of thousands advised in \[@Noda_2024-03-22\].
...
I inspected the block-wise behavioral profiles and RDM structure of the
two top candidate sessions (`755434585` and `756029989`). The full
block-by-block RDM comparison is shown in [Appendix A](#appendix-a).

The key observations were:

- **Session `755434585` Block 1**: Resting with constricted pupils
  majority of the time. The RDM showed minimal structure likely implying
  a drowsy state.
- **Session `755434585` Block 2**: The mouse was actively running and
  RDM showed a richer structure.
- **Session `755434585` combined blocks**: Combining both blocks (20
  ...
  brain states.
- **Session `756029989` Block 1 and Block 2**: The RDM structure was
...
It is a suitable substrate for demonstrating the full estimation
pipeline. Block 1 was used as the analysis block due to lower mean
running speed.
...
Figure 3: Population Response Space for Block 1 of Session 756029989
(Mouse ID - 734865738)
