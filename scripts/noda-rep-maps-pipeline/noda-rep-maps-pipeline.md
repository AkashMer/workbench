...
used for calcium imaging data due to slow decay of the fluorescent
indicator. Neural responses shift at the order of tens to hundreds of
milliseconds which can be captured by Neuropixels probe. Thus similar
binning will fail to capture population level dynamics in the
neuropixels data. Binwise pixel correlation was computed and showed 2
distinct regions in the similarity matrix. This prompted me to apply
KMeans clustering(k=2) to the MDS of the correlation matrix to visualize
the structure. I guessed two clusters before seeing the movie clip based
on the binwise pixel correlation heatmap. The movie clustered into two
structurally distinct halves which can be used for validation of the
neural representational map. Watching the movie directly gave the
context for the 2 clusters. A car dominates the frames from
