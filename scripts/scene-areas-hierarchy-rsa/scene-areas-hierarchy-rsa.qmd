...
A representational map needs to be validated against the physical structure of the stimulus set or a behavioral/psychophysics readout[@Noda_2024-03-22]. Neither is available here so V1 is used as the low-level-feature model for vision. V1's population code is well characterized as encoding retinotopic position and basic visual features (edges, orientation, spatial frequency). Partial RSA regression analysis[@Bosch_2025] is performed on left OPA's RDM against left V1d's RDM, left V2d's RDM & map and walking-direction model RDMs.

```{python}
#| code-fold: true
#| code-summary: "Partial RSA Regression Analysis"
#| label: partial-rsa
